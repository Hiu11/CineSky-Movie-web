  ...
  return (
    <section className="admin-panel admin-table-panel">
      <div className="admin-toolbar">
        <input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Tìm theo tên, email hoặc nội dung" />
      ...
      {activeModule === "feedback" ? (
        <div className="admin-conversation-list">
          {visibleRecords.map((record) => {
            const preview = record.message || record.value;
            ...
            return (
              ...
              >
                <span className="admin-conversation-card__avatar">
                  {(record.fullName || record.email || "?").trim().charAt(0).toUpperCase()}
                </span>
                <span className="admin-conversation-card__body">
                  <span className="admin-conversation-card__top">
                    <strong>{record.fullName || "Khách hàng"}</strong>
                    <em>{record.time}</em>
                  </span>
                  <span className="admin-conversation-card__title">
                    {record.headline || "Góp ý khách hàng"}
                  </span>
                  <span className="admin-conversation-card__preview">
                    {preview}
                  </span>
                </span>
